...
if ($conn->query($sql) === TRUE) {
    ...
    if ($conn->query($sql_tbldata) === TRUE) {
        echo "3. Table 3 created successfully <br/>";
    } else {
        echo "Error creating table: " . $conn->error;
    }
    

    $sql_visiter = "CREATE TABLE IF NOT EXISTS `visiter` (
        `name` varchar(55) NOT NULL,
        `date` date NOT NULL,
        `id` varchar(55) NOT NULL
         )";
      
    if ($conn->query($sql_visiter) === TRUE) {
        echo "3. Table 4 created successfully <br/>";
    } else {
        echo "Error creating table: " . $conn->error;
    }
